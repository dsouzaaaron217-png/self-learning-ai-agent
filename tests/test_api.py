...
class TestAPIEndpoints(unittest.TestCase):
    ...
    def test_quick_capture_api(self):
        res = self.client.post("/api/tasks/quick-capture",
    ...
    def test_feedback_loop_api(self):
        # 1. Create a task
        res_t = self.client.post("/api/tasks", json={"title": "Client meeting with Acme"})
        self.assertEqual(res_t.status_code, 201)
        task_id = res_t.get_json()["task"]["id"]

        # 2. Correct the priority
        res_fb = self.client.post(f"/api/tasks/{task_id}/feedback", json={
            "action": "corrected",
    ...
    def test_backup_export_and_import(self):
        # Create a note
        self.client.post("/api/notes", json={"title": "Test Note", "content": "Important secret content"})
        ...
        res_imp = self.client.post("/api/backup/import", json=backup_data)
        self.assertEqual(res_imp.status_code, 200)
        self.assertTrue(res_imp.get_json()["success"])

    def test_chat_api(self):
        res_chat = self.client.post("/api/chat", json={"message": "What can you do?"})
